...
function display_services(services) {
    ...
    $.each(services, function (i, services) {
        ...
        html = '<div class="col-md-4 mb-3">' +
            ...
            '<button class="btn btn-xs btn-primary shop-item-button m-2" id = "shop-item-button" data-recurring_days="' + services['recurring_days'] + '" data-is_recursive="' + services['is_recursive'] + '" data-business_id="' + services['business_id'] +
            '" data-tax_id=' + services["tax_ids"] + ' data-is_tax_included="' + services['is_tax_included'] + '" data-service_id = "' + services['id'] + '" onclick="add_to_cart_service(event)" type="button">Add to Cart</button>' +
            '</div></div></div>'
