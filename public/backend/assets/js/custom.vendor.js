                ...
                <div class="warehouses">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="number" class="form-control No-negative" id="warehouse_stock" step="0.1" min="0.1" name="warehouses[${$(".variant-item").length}][warehouse_stock][]">
                        </div>
                        <div class="col-md-3">
                            <label for="warehouse_qty_alert">Warehouse Minimum Stock Level</label><span class="asterisk text-danger">*</span>
                            <input type="number" class="form-control No-negative" id="warehouse_qty_alert" step="0.1"  min="0.1" name="warehouses[${$(".variant-item").length}][warehouse_qty_alert][]">
                        </div>
                    </div>
                </div>
            </div>`;
            ...
            if (result.error == true) {
                ...
                Object.keys(result.message).map((key) => {
                    ...
                    $("#create_payment")
                        .find(".modal-body")
                        .prepend(
                            '<div class="alert alert-danger">' + result["message"][key] + "</div>"
                        );
...
if ($("#purchase_form").length > 0) {
    ...
    $(document).ready(function () {
        $('.dropdown-menu .dropdown-item-marker input[data-field="id"]').closest('label').hide();
    });
    ...
    $(document).on("change", ".search_products", function (e) {
        ...
        table_data.image = '<img src="' + site_url + data.image + '" width="60px"  class="img-fluid"/>';
        table_data.sr = count;
        ...
        table_data.price =
            '<input type="number" class="form-control price" step="0.01"  value="' +
            data.purchase_price +
        ...
        $('#purchase_order').on('input', '.price, .discount', function () {
            ...
            if (price <= 0) {
                ...
                row.find('.price').val(price <= 0 ? '' : price); // clear invalid value
            }

            // Validate discount
            if (discount < 0 || discount > price) {
                showToastMessage('Discount must be between 0 and the price.', "error");
                row.find('.discount').val(discount < 0 || discount > price ? '' : discount); // clear invalid value
            }
...
$(function (e) {
    ...
    if ($("#sales_order_form").length > 0) {
        $sales_order_table.on(
            "check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table",
            function () {
                $remove.prop("disabled", !$sales_order_table.bootstrapTable("getSelections").length);
            }
        );
        $remove.click(function (e) {
            e.preventDefault();
            var ids = $.map($sales_order_table.bootstrapTable("getSelections"), function (row) {
                return row.id;
            });
            ...
            var sale_product_id = JSON.parse($('input#sale_product_id').val() || '[]'); // Get current product details (if any)

            // Filter out products that have an ID matching the removed rows
            var updatedSale_product_id = sale_product_id.filter(function (sale_product_id) {
...
function purchase_total() {
    ...
    for (var i = 0; i < length; i++) {
        var price = parseFloat($(".table_price")[i].innerHTML);
        total += isNaN(price) ? 0 : price;
    }

    // Get tax-related data
    let order_taxs = [];
    if (document.getElementById('order_taxes')) {

        if ($("#order_taxes").val().length > 0) {
            order_taxs = JSON.parse($("#order_taxes").val())
        }
    }
    ...
    if (order_taxs.length > 0) {
        let tax = 0;
        order_taxs.forEach(taxItem => {
            let tax_percentage = parseFloat(taxItem.percentage) / 100;
            tax += final_total * tax_percentage;
        });
        final_total += parseFloat(tax.toFixed(3));
    }
...
$(document).on("show.bs.modal", "#barcode_Modal", function display_barcode() { });
...
function display_barcode(data) {
    ...
    for (i = 0; i < total_variants; i++) {
        ...
        var variant_name = variants[i]["barcode"]?.trim() || variants[i]["variant_name"];
...
$(document).ready(function () {
    ...
    if (document.getElementById('purchase_form')) {
        $.ajax({
            type: "POST",
            url: site_url + "admin/tax/get_taxs",
            success: function (response) {
...
function printInvoice() {
    ...
    var printWindow = window.open(base_url + '/admin/invoices/thermal_print/' + id, '_blank');
...
function get_todays_stats() {
    ...
    fetchAndSetValue("today_sales", "/admin/todays_total_sales", "total_amount", (val) => total_sale = val);
    fetchAndSetValue("today_purchase", "/admin/todays_total_purchase", "total_amount", (val) => total_purchase = val);
    fetchAndSetValue("today_expanse", "/admin/get_todays_expense", "total_amount", (val) => total_expens = val);
    fetchAndSetValue("today_payments", "/admin/todays_total_payment_resived", "total_amount");
    fetchAndSetValue("today_payments_remaining", "/admin/todays_total_payment_remaining", "diffrence");
    fetchAndSetValue("today_paid", "/admin/todays_total_paids", "total_amount");
    fetchAndSetValue("today_amount_to_pay", "/admin/todays_total_remaining", "diffrence");
