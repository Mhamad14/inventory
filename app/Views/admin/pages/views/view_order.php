...
<div class="row mb-3">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="returnItemsModalLabel"><?= labels('return_items', 'Return Items') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="process_return_form" method="post" action="<?= base_url('admin/orders/process_return') ?>" id="process_return_form">
                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th width="10%"><?= labels('image', 'Image') ?></th>
                                    <th width="30%"><?= labels('product_name', 'Product Name') ?></th>
                                    <th width="15%"><?= labels('price', 'Price') ?></th>
                                    <th width="15%"><?= labels('ordered', 'Ordered') ?></th>
                                    <th width="15%"><?= labels('already_returned', 'Already Returned') ?></th>
                                    <th width="15%"><?= labels('return_quantity', 'Return Quantity') ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($items as $item) { 
                                    ...
                                    if ($returnable_qty <= 0) continue; // Skip if nothing can be returned
                                ?>
                                <tr>
                                    <td>
                                        <img alt="image" src="<?= (!empty($item['image'])) ? base_url($item['image']) : "" ?>" class="img-fluid" style="max-height: 50px;">
                                    ...
                                    <td>
                                        <select name="return_quantity[<?= $item['id'] ?>]" class="form-control return-quantity-select" data-item-id="<?= $item['id'] ?>" data-price="<?= $item['price'] ?>">
                                            <option value="0">0</option>
                                            <?php for($i = 1; $i <= $returnable_qty; $i++) { ?>
                    ...
                    <div class="form-group mt-3">
                        <label for="return_reason"><?= labels('return_reason', 'Return Reason') ?></label><span class="asterisk text-danger"> *</span>
                        <textarea class="form-control" id="return_reason" name="return_reason" rows="3" required></textarea>
                    </div>
                    
                    <div class="return-summary mt-4">
                        <h5><?= labels('return_summary', 'Return Summary') ?></h5>
                        <div class="calculation-item">
                            <div class="item-label"><?= labels('total_return_amount', 'Total Return Amount') ?></div>
                            <div class="item-value" id="return-total-amount"><?= currency_location("0") ?></div>
                        </div>
                    </div>
                    
                    <button type="submit" class="btn btn-primary" id="submitReturn" name="submitReturn" value="Submit"><?= labels('submit_return', 'Submit Return') ?></button>
                    <div class="mt-3">
                        <div id="return-form-result"></div>
                    </div>
                </form>
            </div>
        </div>
...
$(document).ready(function() {
    // Calculate return total when quantities change
    $('.return-quantity-select').on('change', function() {
        calculateReturnTotal();
    });
    
    // Calculate and update return total
    function calculateReturnTotal() {
        let totalReturnAmount = 0;
        
        $('.return-quantity-select').each(function() {
        ...
        // Update the display
        $('#return-total-amount').text('$ ' + totalReturnAmount.toFixed(2));
    }
    
    // Form submission
    $('#process_return_form').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            data: $(this).serialize(),
            beforeSend: function() {
                $('#submitReturn').html('Processing...');
                $('#submitReturn').attr('disabled', true);
            },
            success: function(result) {
                $('#submitReturn').html('<?= labels('submit_return', 'Submit Return') ?>');
                $('#submitReturn').attr('disabled', false);
                console.log("Return response:", result);
                
                if (result.error == false) {
                    $('#return-form-result').html('<div class="alert alert-success">' + result.message + '</div>');
                    setTimeout(function() {
                        location.reload();
                    }, 1500);
                } else {
                    $('#return-form-result').html('<div class="alert alert-danger">' + result.message + '</div>');
                }
            },
            error: function(xhr) {
    $('#submitReturn').html('<?= labels('submit_return', 'Submit Return') ?>');
    ...
    $('#return-form-result').html('<div class="alert alert-danger">Error: ' + xhr.responseText + '</div>');
