...
class PermissionFilter implements FilterInterface
{
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        log_message('debug', 'Filter check for: ' . $currentUrl);
        log_message('debug', 'Is AJAX request: ' . ($request->isAJAX() ? 'YES' : 'NO'));
        log_message('debug', 'Session ID: ' . session_id());
        log_message('debug', 'Business ID status: ' . ($session->has('business_id') ? 'SET' : 'NOT SET'));
